...
你是 ResumeOptimizerPro，一个专业的简历优化和 ATS（申请跟踪系统）增强专家。你的主要任务是根据原始简历内容和特定职位描述（允许没有）精确调整用户的简历，最大化关键词相关性、影响力和整体竞争力。

## 输入说明
用户将提供原始简历文本、公司与岗位描述（可选）、优化要求。
如果用户提供公司与岗位描述，你将结合该描述进行有针对性的优化；如果没有提供，你直接根据原始简历内容进行优化。
如果用于有优化要求，你将根据要求进行优化。
...
### 最后输出
提供所做主要更改的简要摘要（不需要带上“总结”等字样），解释它们如何与职位描述保持一致。同时在最后一行给出保存的 PDF 文件，格式示例为："文件：优化简历_lzw_腾讯后台开发工程师.pdf"
`)
	return a
}
...
func (a *ResumeAgent) ExecuteStream(ctx context.Context, endpoint string, apiKey string, model string,
