...
// travelWorkflowName 旅游攻略工作流名称
const travelWorkflowName = "AI 旅游攻略生成"

// SeedTravelWorkflow 初始化旅游攻略工作流种子数据
func SeedTravelWorkflow(db *gorm.DB) error {
	var count int64
	db.Model(&domain.AgentFlow{}).Where("name = ?", travelWorkflowName).Count(&count)
	if count > 0 {
		log.Info("旅游攻略工作流已存在，跳过初始化")
		return nil
	}

	topology := buildTravelTopology()

	flow := &domain.AgentFlow{
		Name:             travelWorkflowName,
		Description:      "根据目的地信息，自动生成全面、详细、图文并茂的旅游攻略，包含路线规划、景点介绍、美食推荐、住宿建议等，并导出为 PDF 文件。",
		Topology:         topology,
		CurrentVersion:   1,
		PublishedVersion: 1,
		Status:           "published",
	...
	if err := db.Create(flow).Error; err != nil {
		log.Error("创建旅游攻略工作流失败", zap.Error(err))
		return err
	}

	log.Info("旅游攻略工作流初始化成功", zap.Int64("id", flow.Id))
	return nil
}

func buildTravelTopology() string {
	travelTools := []string{
		"web_search_tool",
		"image_search_tool",
		"image_download_tool",
	...
	topology := map[string]interface{}{
		...
		"edges": []map[string]interface{}{
			{"id": "edge_start_agent", "source": "start_1", "target": "agent_travel"},
			{"id": "edge_agent_end", "source": "agent_travel", "target": "end_1"},
		},
