...
- tags 使用 2-5 个简体中文短词，并优先从内定分类标签中挑选（匹配几个算几个，允许一个都不匹配）：实用工具、AI、Skill、开源项目、开发框架、开发工具、文档教程、设计资源；在此基础上可补充 1-2 个有辨识度的其他标签（如技术名 "Go"、"Vue" 或领域词）。GitHub topics 需翻译为对应中文（如 "developer-tools"→"开发工具"），通用专有名词保留英文（如 "Go"、"Vue"）。
