...
> UtilsResponse apiChatConversationListPost(opts)
...
let apiInstance = new TxingAiApi.DefaultApi();
let opts = {
  'cursor': "cursor_example", // String | 游标
  'pageSize': 20 // Number | 每页大小
};
apiInstance.apiChatConversationListPost(opts).then((data) => {
  console.log('API called successfully. Returned data: ' + data);
}, (error) => {
  console.error(error);
});
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **cursor** | **String**| 游标 | [optional] 
 **pageSize** | **Number**| 每页大小 | [optional] [default to 20]
